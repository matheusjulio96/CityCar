...
public class AcessoDados extends SQLiteOpenHelper {
    ...
    public AcessoDados(Context context) {
        super(context, "citycarbd", null, 1);
    ...
    public void onCreate(SQLiteDatabase banco) {//todas as tabelas vão ser criadas através do método onCreate
        ...
        DDL = "CREATE TABLE veiculo(placa TEXT PRIMARY KEY, marca TEXT, modelo TEXT," +
                "ano TEXT, combustivel TEXT,chassi TEXT, locado BOOLEAN)";
        banco.execSQL(DDL);
        ...
        DDL = "CREATE TABLE solicitacao(cpf_usuario INTEGER references usuario, motivo TEXT, periodo INTEGER," +
                "p_dias BOOLEAN, p_horas BOOLEAN, hora_ideal TEXT, deferido BOOLEAN, placa_veic TEXT references veiculo)";
        banco.execSQL(DDL);
    }
    ...
    public void alteraVeículo(Veiculo veiculo){
        SQLiteDatabase banco = this.getWritableDatabase();
        ContentValues valores = new ContentValues();
        valores.put("locado", veiculo.isLocado());
        //banco.insert("veiculo", null, valores);
        banco.update("veiculo",valores,"placa = '"+veiculo.getPlaca()+"'", null);
        banco.close();
    }
...
public class AcessoDados extends SQLiteOpenHelper {
    ...
    public void onCreate(SQLiteDatabase banco) {//todas as tabelas vão ser criadas através do método onCreate
        ...
        //nao precisa de autoincrement pq sqlite cria rowid automaticamente
        DDL = "CREATE TABLE solicitacao(cpf_usuario INTEGER references usuario, motivo TEXT, periodo INTEGER," +
                "p_dias BOOLEAN, p_horas BOOLEAN, hora_ideal TEXT, deferido BOOLEAN, localRetirada TEXT,horaRetirada TEXT,kmRetirada ,placa_veic TEXT references veiculo)";
        banco.execSQL(DDL);
    }
    ...
    * */
    ...
    public ArrayList<AprovaSolicitacao> listaSolicitacoes (ArrayList<AprovaSolicitacao> lista){
        ...
        try{
            SQLiteDatabase db = this.getReadableDatabase();
            campo = db.rawQuery("select cpf_usuario,motivo,nome, solicitacao.rowid, hora_ideal,setor, periodo,p_dias, p_horas from solicitacao inner join usuario on solicitacao.cpf_usuario = usuario.cpf " +
                    "and deferido = 0",null);
            if (campo !=null) {
                //campo.moveToFirst();
                while (true) {
    ...
    public void aprovarSolicitacao(AprovaSolicitacao solicitacao){
